...
class EnvRelativeDemand(EnvBasic):
    ...
    def __init__(self, data, train=True, window_size=384, obs_len=10):
        super(EnvRelativeDemand, self).__init__(
            data, train, window_size, obs_len)
        self.observation_space = spaces.Box(
...
class EnvComplexR(EnvBasic):
    ...
    def _gen_cost(self):
        ...
        dft2['a_total_award'] = 7 + dft2['a_downtime_award'] + \
            dft2['a_congestion_award'] + \
            dft2['a_environmental_award'] + dft2['a_fix_award']

        dft2['n_leakage_award'] = - \
            dft2['WaterDemandWithLeaks'].rolling(window=12).sum().shift(-11)
        dft2['n_environmental_award'] = - \
            np.log1p(dft2['LeakageLabel'].rolling(window=30*24*4).sum())
        dft2['n_total_award'] = dft2['n_leakage_award'] + \
            dft2['n_environmental_award'] - 7
    ...
    def _calculate_reward(self, action):
        ...
        n_total_award = self.processed_data['n_total_award'].iloc[self.current_step].round(
            2)
        a_total_award = self.processed_data['a_total_award'].iloc[self.current_step].round(
            2)

        if action == 1:
            if leak_present:
                return a_total_award  # leak detected
            else:
